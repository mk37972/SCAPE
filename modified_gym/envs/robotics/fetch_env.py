...
class FetchEnv(robot_env.RobotEnv):
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        reward = -(d > self.distance_threshold).astype(np.float32) - np.float32(fragile_goal) * 2 if self.pert_type != 'none' else -(d > self.distance_threshold).astype(np.float32)
        return reward
    ...
    def _set_action(self, action):
        ...
        if self.model_path.find('Chip') == -1:
            ...
            if action.shape[0] > 3:
                ...
                self.psv_prev_stiffness += psv_stiffness_ctrl
                self.psv_prev_stiffness = np.max([np.min([self.psv_prev_stiffness, self.max_stiffness]), 10.0])
    ...
    def _get_obs(self):
        ...
        ind = 1 if self.model_path.find('Chip') == -1 else 0
        ...
        if self.model_path.find('Chip') == -1:
            ...
            if self.pert_type != 'none' and self.pert_type != 'meas':
                if np.linalg.norm(object_rel_pos) < 0.03: 
                    self.sim.data.qvel[self.sim.model.joint_name2id('object0:joint')+ind] += np.random.random()*1.0-0.5
